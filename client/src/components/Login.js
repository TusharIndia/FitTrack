...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const response = await fetch("https://fit-track-api.vercel.app/api/user/login", {
      method: "POST",
      headers: {
      ...
      <div className="flex items-center justify-center p-6 lg:p-10">
        <div className="mx-auto w-full max-w-md space-y-6">
          ...
          <p className="text-center text-sm text-muted-foreground">
            Don't have an account? <Link to="/signup" className="text-primary font-medium underline underline-offset-4 hover:text-primary/80">Sign Up</Link>
          </p>
        </div>
      </div>
    </div>
